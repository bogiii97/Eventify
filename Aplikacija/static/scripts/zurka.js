...
$(document).ready(function () {
    ...
    $('#naziv').on('keyup', (e) => {
        ...
        const vrsta_muzike = $("#vrsta_muzike").val();
        const grad = $("#grad").val();

        if (jedno_postavljeno()) {
            saljiZahtev(nazivVrednost, izvodjac, date_range, vrsta_muzike, grad);
        } else {
    ...
    $('input[name="daterange"]').on('change', (e) => {
        ...
        saljiZahtev(naziv, izvodjac, date_range, vrsta_muzike, grad);
    });
    /**
     *Implementirao: Vladan Vasic, 2020/0040
    ...
    /**
     *Implementirao: Vladan Vasic, 2020/0040
     * Ovo je funkcija koja se izvrsava kada se promeni vrednost polja za pretragu Vrsta muzike
     * Tada se salje asinhroni POST request ka url-u pozoriste_pretraga koji vraca odgovarajuce dogadjaje koji prolaze filter
     * */
    $('#vrsta_muzike').on('keyup', (e) => {
        const nazivVrednost = $("#naziv").val();
        const date_range = $("input[name='daterange']").val();
        const izvodjac = $("#izvodjac").val();
        const vrsta_muzike = e.target.value;
        const grad = $("#grad").val();
...
function jedno_postavljeno() {
    ...
    return $('input[name="daterange"]').val() !== def || nazivVrednost !== '' || izvodjac !== '' || vrsta_muzike !== '' || grad !== '';
}
/**
     * Implementirao: Vladan Vasic, 2020/0040
                        ...
                        $('.sa-pretragom').append($(`<div class="row m-2 rounded border event-info">
                             <div class="col-sm-2 p-0 text-start">
                                ...
                                <div class="col-sm-10 event-text"> 
                                    ...
                                    <p class="mb-1"><strong>Vrsta muzike: </strong>${item.vrsta_muzike}</p>
...
/**
     ...
     * */
function saljiZahtev(naziv, izvodjac, date_range, vrsta_muzike, grad){
    fetch('/zurka_pretraga/', {
                body: JSON.stringify({
                    naziv: naziv,
                    izvodjac: izvodjac,
                    date_range: date_range,
                    vrsta_muzike: vrsta_muzike,
                    grad: grad,
                }),
                method: 'POST',
            })
