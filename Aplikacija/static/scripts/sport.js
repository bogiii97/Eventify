...
$(document).ready(function () {
    ...
    $('#naziv').on('keyup', (e) => {
        ...
        const nazivSport = $("#naziv_sporta").val();
        const grad = $("#grad").val();

        if (jedno_postavljeno()) {
            saljiZahtev(nazivVrednost, nazivSport, date_range, grad);
        } else {
    ...
    $('input[name="daterange"]').on('change', (e) => {
        ...
        saljiZahtev(naziv, nazivSport, date_range, grad);
    });
    /**
     *Implementirao: Vladan Vasic, 2020/0040
     * Ovo je funkcija koja se izvrsava kada se promeni vrednost polja za pretragu Naziv sporta
     * Tada se salje asinhroni POST request ka url-u pozoriste_pretraga koji vraca odgovarajuce dogadjaje koji prolaze filter
     * */
    $('#naziv_sporta').on('keyup', (e) => {
        const nazivVrednost = $("#naziv").val();
        const date_range = $("input[name='daterange']").val();
        const nazivSport = e.target.value;
        const grad = $("#grad").val();
...
function jedno_postavljeno() {
    const nazivVrednost = $("#naziv").val();
    const nazivSportVrednost = $("#naziv_sporta").val();
    const grad = $("#grad").val();
    let trenutniDatum = new Date();
    let dan = trenutniDatum.getDate();
    ...
    let def = mesec.toString().padStart(2, '0') + '/' + dan.toString().padStart(2, '0') + '/' + godina + ' - ' + mesec.toString().padStart(2, '0') + '/' + dan.toString().padStart(2, '0') + '/' + godina;
    return $('input[name="daterange"]').val() !== def || nazivVrednost !== '' || nazivSportVrednost !== '' || grad !== '';
}
/**
     * Implementirao: Vladan Vasic, 2020/0040
                        ...
                        $('.sa-pretragom').append($(`<div class="row m-2 rounded border event-info">
                             <div class="col-sm-2 p-0 text-start">
                                ...
                                <div class="col-sm-10 event-text"> 
                                    ...
                                    <p class="mb-1"><strong>Učesnici: </strong>${item.ucesnici}</p>
                                    <p class="mb-1"><strong>Naziv sporta: </strong>${item.naziv_sporta}</p>
...
/**
     ...
     * */
function saljiZahtev(naziv, naziv_sporta, date_range, grad){
    fetch('/sport_pretraga/', {
                body: JSON.stringify({
                    naziv: naziv,
                    naziv_sporta: naziv_sporta,
